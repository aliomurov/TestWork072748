        ...
        <v-row justify="center">
            <v-col cols="11">
                <div>
                    <div>
                        <br>
                        <v-toolbar-title>{{administrator.name}}</v-toolbar-title>
                        <br>
                        <v-form v-model="valid">
                            ...
                            <v-col cols="12" sm="12">
                                <v-text-field color="error" type="password" v-model="administrator.password" label="Пароль"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-text-field :rules="[passwordMatch]" color="error" type="password" v-model="administrator.rpassword" label="Повторите Пароль"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-select
                                    :rules="roleRules"
...
<script>
    export default {
        ...
        methods: {
            initialize () {
                axios.get(`/api/admin/administrators/edit/${this.$route.params.id}`)
                    .then(res => {
            ...
            update() {
                axios.post(`/api/admin/administrators/update/${this.$route.params.id}`, this.administrator)
                    .then(res => {
                        this.$router.push({name: 'administrators.index'});
                    })
                    .catch(err =>{
